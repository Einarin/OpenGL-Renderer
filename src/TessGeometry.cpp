...
{}
	...
	bool Face::buildLod(int maxTesselation){
		for (int xTess = 1; xTess < maxTesselation; xTess++){
			for (int yTess = 1; yTess < maxTesselation; yTess++){
				VertexBufferBuilder builder;
				builder.vertexCount((xTess + 1)*(yTess + 1));
				builder.hasNormal(true);
				builder.hasTexCoord3D(true);
				VertexBuffer buffer = builder.build();
				int i = 0;
				...
				buffer.configureAttributes();
				glBufferData(GL_ARRAY_BUFFER, buffer.sizeInBytes(), buffer.buffPtr(), GL_STATIC_DRAW);
