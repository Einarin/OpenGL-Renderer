...
vec2 st = vec2(texCoords.s,texCoords.t*5.0);
...
void main( void )
{
  ...
  dt *= 1.5;
  //cellular(64.0*(st+dt), F, d1d2); // Returns vectors to points
  //pattern += F.x;
  dt *= 1.5;
  ...
  gl_FragColor = pattern * vec4(0.8,0.25,0.0,0.0) + pattern2*vec4(0.5,0.0,0.0,0.0) + vec4(0.5,0.2,0.0,1.0);
}