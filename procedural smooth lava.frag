...
vec2 st = vec2(texCoords.s,texCoords.t*2.0);
...
void main( void )
{
  ...
  vec4 dc = vec4(2.0,0.05,0.0,1.0);
  vec4 ic = vec4(2.0,5.0,0.0,0.0);
  
  color += dc * snoise(vec3(0.005*st+dt+ft,50.0));
  dt *= 0.75;
  dc *= ic;
  color += 0.5 * dc *snoise(vec3(0.05*st+dt+ft,60.0));
  dt *= 0.75;
  dc *= ic;
  color += 0.25* dc * snoise(vec3(st+dt+ft,70.0));
  dt *= 0.75;
  dc *= ic;
  color += 0.125* dc*snoise(2.0*vec3(st+dt+ft,80.0));
  dt *= 0.75;
  dc *= ic;
  color += 0.0625* dc*snoise(vec3(5.0*st+dt+ft,90.0));
  //color.g = max(color.g,0.2);
  //color.r = max(color.r,color.g);
  color.rg *= 0.15;
  color.rg += 0.5;
  color.r = max(color.r,0.8);
  //float n = snoise(vec3(10.0*st.s,10.0*st.t,60.0));
  //vec4 pixel = vec4(0.4*vec2(n) + 0.5,0.0,1.0);
  
  //vec4 pixel = (0.4*n+0.8)*vec4(0.6,0.3,0.0,1.0);
  //pixel.rg += max(vec2(pattern,0.5*pattern),vec2(0.5,0.2));
  //pixel.r += aastep(thresh, f);
  color.a = 1.0;
  //color = (snoise(vec3(5.0*st,50.0))) * vec4(0.5,0.2,0.0,1.0);
  gl_FragColor = color;
}